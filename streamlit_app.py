...
def particle_analysis_auto(image, filename, n_color_groups=5, min_area=10):
...
# --- STREAMLIT UI ---
st.title("🧪 Smart Particle Analyzer")
st.write("Upload an image to automatically analyze particles based on color and area.")
...
if uploaded_file:
    ...
    min_area = st.slider("Minimum Particle Area", 1, 500, 20)

    with st.spinner("Analyzing..."):
        rows, total_particles, overlay = particle_analysis_auto(
            image,
    ...
    st.write(f"**{total_particles}** detected before grouping.")

    if overlay is not None:
        st.subheader("🖼️ Detected Particle Overlay")
        st.image((overlay * 255).astype(np.uint8), caption="Overlay", use_container_width=True)

    if rows:
        st.subheader("📊 Particle Summary")
        st.dataframe(pd.DataFrame(rows))
    else:
        st.warning("No particles matched current filtering. Try lowering min area.")
